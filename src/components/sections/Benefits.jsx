                    ...
                    {benefits.map((benefit, idx) => (
                        <div key={idx} className="opacity-0 animate-[fadeInUp_0.6s_ease-out_forwards]" style={{ animationDelay: `${(idx + 1) * 0.1}s` }}>
                            <Card variant="standard" className="text-center h-full flex flex-col items-center">
                                <div className="w-16 h-16 bg-gray-light rounded-full flex items-center justify-center text-2xl mb-6 shadow-subtle group-hover:scale-110 transition-transform duration-300">
                                    {benefit.icon}
                                </div>
